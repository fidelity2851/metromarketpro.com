<div class="col py-4 px-0 px-md-3">
    ...
    <div class="col d-lg-flex justify-content-between align-items-center mb-5">
        ...
        <div class="col col-lg-9 col-xl-8 d-md-flex justify-content-between px-0 ml-auto">
            <div class="col acct_dep_cont d-flex align-items-center shadow-sm mb-4 mb-md-0 mr-md-5">
                ...
                <div class="">
                    <p class="acct_dep_header">${{number_format($this->totalWithdrawal())}}</p>
                    <p class="acct_dep_text">Total Withdrawal</p>
                </div>
            </div>
            <div class="col acct_dep_cont d-flex align-items-center shadow-sm">
                <span class="acct_dep_hint2">Pending</span>
                <span class="acct_dep_icon"> <i class="fas fa-hand-holding-usd mr-4"></i> </span>
                <div class="">
                    <p class="acct_dep_header">${{number_format($this->pendingWithdrawal())}}</p>
                    <p class="acct_dep_text">Pending Withdrawal</p>
                </div>
            </div>
        </div>
    </div>
    ...
    <div wire:loading
        wire:target='search,sort_by,order_by,per_page,DeleteWithdrawal,DisapproveWithdrawal,ApproveWithdrawal'
        class="col">
        ...
        <table class="table table-bordered table-hover table-striped">
            <thead>
                <tr>
                    ...
                    @endcan
                    <th class="table_head" scope="col">Withdrawal Method</th>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    @endcan
                    <td class="table_data">{{ $item->withdrawal_type }}</td>
                    ...
                    <td class="table_data">
                        ...
                        @can('adminOnly')
                        @if ($item->status != true)
                        <button wire:click="ApproveWithdrawal({{$item->id}})" type="button"
                            class="table_btn5">Approve</button>
                        @endif
                        @if ($item->status == true)
                        <button wire:click="DisapproveWithdrawal({{$item->id}})" type="button"
                            class="table_btn3">Disapprove</button>
                        @endif
                        @if ($item->status != true)
                        <button
                            onclick="return confirm('Are you sure you want to delete?')  || event.stopImmediatePropagation()"
                            wire:click="DeleteWithdrawal({{$item->id}})" type="button"
                            class="table_btn4">Delete</button>
                        @endif
                        @endcan
                        <button wire:click="WithdrawalInvoiceEvent({{$item->id}})" type="button" class="table_btn1"

<form wire:submit.prevent="MakeInvestment" method="POST" class="modal-content">
    <div class="modal-header">
        <h5 class="acct_modal_header" id="staticBackdropLabel">New Investment</h5>
    ...
    <div class="modal-body">
        <div class="">
            ...
            @endcan
            <div class="mb-3">
                <label class="acct_modal_label">Investment Plan
                ...
                <select class="acct_modal_sel" wire:model.live="investment_plan">
                    <option value="">Select Your Perferred Plan</option>
                    @foreach ($plans as $item)
                    <option value="{{$item->id}}">{{$item->title}}
                        (${{number_format($item->min_investment)}} - ${{number_format($item->max_investment)}})</option>
                    @endforeach
                </select>
            ...
            <div class="col rounded bg-light py-2 my-2">
                <div class="d-md-flex justify-content-between">
                    <p class="acct_modal_head">Min Amount: {{$GlobalSettings->currency}}{{
                        number_format($item->min_investment) }}</p>
                    <p class="acct_modal_head">Max Amount: {{$GlobalSettings->currency}}{{
                        number_format($item->max_investment) }}</p>
                </div>
                <hr>
                <p class="acct_modal_head">Interest Rate: <strong>
                        {{ $item->rate_type == 'fixed' ? $GlobalSettings->currency : ''}}
                        {{$item->rate_number}}{{$item->rate_type == 'percent' ? '%' : ''}}
                    </strong></p>
            ...
            @endforeach
            <div class="mb-3">
                <label class="acct_modal_label">Enter Ammount</label>
    ...
    <div class="modal-footer">
        ...
        <button wire:loading.remove wire:target="MakeInvestment" type="submit" class="acct_modal_btn">Submit</button>
        <button wire:loading wire:target="MakeInvestment" type="button" class="acct_modal_btn disabled"

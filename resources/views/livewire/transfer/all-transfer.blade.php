<div class="col py-4 px-0 px-md-3">
    ...
    <div class="col d-lg-flex justify-content-between align-items-center mb-5">
        ...
        <button type="button" class="acct_cont_btn mb-4 mb-lg-0" data-toggle="modal" data-target="#staticBackdrop">New
            Transfer</button>
        @endcannot
        <div class="col col-lg-9 col-xl-8 d-md-flex justify-content-end px-0 ml-auto">
            <div class="col col-lg-6 acct_dep_cont d-flex align-items-center shadow-sm mb-4 mb-md-0">
                <span class="acct_dep_hint">Active</span>
                <span class="acct_dep_icon mr-4"> <i class="fas fa-upload "></i> </span>
                <div class="">
                    <p class="acct_dep_header">${{number_format($this->totalTransferOut())}}</p>
                    <p class="acct_dep_text">Transfer Out</p>
                </div>
            </div>
            @can('userOnly')
            <div class="col col-lg-6 acct_dep_cont d-flex align-items-center shadow-sm mb-4 mb-md-0 ml-xl-5">
                <span class="acct_dep_hint">Active</span>
                <span class="acct_dep_icon mr-4"> <i class="fas fa-download "></i> </span>
                <div class="">
                    <p class="acct_dep_header">${{number_format($this->totalTransferIn())}}</p>
                    <p class="acct_dep_text">Transfer In</p>
                </div>
            </div>
            @endcan
        </div>
        ...
        <table class="table table-bordered table-hover table-striped">
            <thead>
                <tr>
                    ...
                    <th class="table_head" scope="col">Sender</th>
                    <th class="table_head" scope="col">Receiver</th>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td class="table_data">{{ $item->sender->fullname }}</td>
                    <td class="table_data">{{ $item->receiver->fullname }}</td>
                    <td class="table_data">${{ $item->sender->id == Auth::user()->id ? '-' : '+' }}{{ number_format($item->amount) }}</td>
                    <td class="table_data">
                        @switch($item->status)
                        @case(true)
                    ...
                    <td class="table_data">
                        <button wire:click="TransferInvoiceEvent({{$item->id}})" type="button" class="table_btn1"
